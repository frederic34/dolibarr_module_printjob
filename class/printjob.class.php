...
/**
 * \file        class/printjob.class.php
 * \ingroup     printjob
 * \brief       This file is a CRUD class file for PrintJob (Create/Read/Update/Delete)
 */
 ...
 * Class for PrintJob
 */
class PrintJob extends CommonObject
{
	/**
	 * @var string 		ID of module.
	 */
	public $module = 'printjob';

	/**
	 * @var string 		ID to identify managed object.
	 */
	public $element = 'printjob';

	/**
	 * @var string		Prefix to check for any trigger code of any business class to prevent bad value for trigger code.
	 * @see CommonTrigger::call_trigger()
	 */
	public $TRIGGER_PREFIX = 'PRINTJOB';	// Will be used to build trgiger keys 'PRINTJOB_MYOBJECT_MODIFY', ...
	...
	/**
	 * @var string 		String with name of icon for printjob. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'printjob@printjob' if picto is file 'img/object_printjob.png'.
	 */
	public $picto = 'fa-printer';
	...
	public $fields = array(
		...
		"printerid" => array("type" => "varchar(255)", "label" => "PrinterId", "enabled" => "1", 'position' => 20, 'notnull' => 1, "visible" => "1", "index" => "1", "searchall" => "1", "showoncombobox" => "1", "validate" => "1", "comment" => "Reference of object"),
		"filename" => array("type" => "varchar(255)", "label" => "Filename", "enabled" => "1", 'position' => 30, 'notnull' => 0, "visible" => "1", "alwayseditable" => "1", "searchall" => "1", "css" => "minwidth300", "cssview" => "wordbreak", "help" => "Help text", "showoncombobox" => "2", "validate" => "1",),
		"modulepart" => array("type" => "text", "label" => "ModulePart", "enabled" => "1", 'position' => 60, 'notnull' => 0, "visible" => "3", "validate" => "1",),
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('printjob', 'printjob', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('printjob', 'printjob', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('printjob', 'printjob_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.$this->db->prefix()."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'printjob/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'printjob/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.$this->db->prefix()."ecm_files set filepath = 'printjob/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'printjob/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->printjob->dir_output.'/printjob/'.$oldref;
				$dirdest = $conf->printjob->dir_output.'/printjob/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->printjob->dir_output.'/printjob/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('printjob','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('printjob','printjob_advance','validate'))))
		 {
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'PRINTJOB_MYOBJECT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'PRINTJOB_MYOBJECT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'PRINTJOB_MYOBJECT_REOPEN');
	}
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowPrintJob")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("PrintJob").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$baseurl = dol_buildpath('/printjob/printjob_card.php', 1);
		$query = ['id' => $this->id];
		if ($option !== 'nolink') {
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowPrintJob");
	...
	public function getLinesArray()
	{
		...
		$objectline = new PrintJobLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_printjob:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('PRINTJOB_MYOBJECT_ADDON')) {
			$conf->global->PRINTJOB_MYOBJECT_ADDON = 'mod_printjob_standard';
		}

		if (getDolGlobalString('PRINTJOB_MYOBJECT_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('PRINTJOB_MYOBJECT_ADDON').".php";
			$classname = getDolGlobalString('PRINTJOB_MYOBJECT_ADDON');

			// Include file with class
			$dirmodels = array_merge(array('/'), (array) $conf->modules_parts['models']);
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/printjob/");
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefPrintJob $obj';
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			if (!empty($this->model_pdf)) {
				...
				$modele = getDolGlobalString('MYOBJECT_ADDON_PDF', 'standard_printjob');
			}
		}

		$modelpath = "core/modules/printjob/doc/";
...
/**
 * Class PrintJobLine. You can also remove this and generate a CRUD class for lines objects.
 */
class PrintJobLine extends CommonObjectLine
{
	// To complete with content of an object PrintJobLine
	// We should have a field rowid, fk_printjob and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'printjob'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_printjob'
