...
/**
 *   	\file       printjob_list.php
 *		\ingroup    printjob
 *		\brief      List page for printjob
 */
...
include_once __DIR__.'/class/printjob.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('printjob', 'printjob', 'write');
	$permissiontodelete = $user->hasRight('printjob', 'printjob', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("printjob")) {
	accessforbidden('Module printjob not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	$objectlabel = 'PrintJob';
	$uploaddir = $conf->printjob->dir_output;
...
$title = $langs->trans("PrintJobs");
//$help_url = "EN:Module_PrintJob|FR:Module_PrintJob_FR|ES:Módulo_PrintJob";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/printjob/printjob_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-printjob page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for a horizontal scroll in the table instead of page
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/printjob/printjob_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendPrintJobRef";
$modelmail = "printjob";
$objecttmp = new PrintJob($db);
